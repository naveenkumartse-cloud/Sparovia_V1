...
import { Globe, ArrowUpRight, CheckCircle2, Layers, Image as ImageIcon, Sparkles, Loader2 } from 'lucide-react';
import { websiteApi, WebsiteContent } from '@/lib/api/website';

export default function WebsitePage() {
  const { activeBusiness } = useBusiness();

  const [websiteSections, setWebsiteSections] = React.useState<WebsiteContent[]>([]);
  ...
  React.useEffect(() => {
    const loadContent = async () => {
      try {
        setIsLoading(true);
        const data = await websiteApi.getContent(activeBusiness!.id);
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-slate-500">{websiteSections.length} Active Sections</span>
          </div>
        </CardHeader>
            ...
            websiteSections.map((sec, idx) => (
              <div key={idx} className="py-3.5 flex items-center justify-between hover:bg-slate-50/50 px-2 rounded-lg transition-colors">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="text-sm font-semibold text-slate-900">{sec.sectionName}</span>
                    <span className="ml-2 text-xs text-slate-500">({sec.itemsCount} content items)</span>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  <Badge variant="slate">{sec.sectionType}</Badge>
