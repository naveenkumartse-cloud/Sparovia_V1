...
import { Image as ImageIcon, Upload, Sparkles, Folder, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function MediaPage() {
  ...
  React.useEffect(() => {
    const loadMedia = async () => {
      try {
        setIsLoading(true);
        const data = await mediaApi.getMedia(activeBusiness!.id);
        setMediaList(data);
      } catch (error) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0 || !activeBusiness) return;

    try {
      setIsUploading(true);
      const file = files[0];
      const category = filterCategory === 'All' ? 'Portfolio' : filterCategory;
      
      const newAsset = await mediaApi.uploadMedia(activeBusiness.id, file, category);
            ...
            <Card key={item.id} hover className="overflow-hidden p-0 flex flex-col">
                ...
                <img
                  src={
                    item.url.startsWith('http') || item.url.startsWith('blob:') 
                      ? item.url 
                      : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}${item.url}`
                  }
                  alt={item.title}
              ...
              <div className="p-4 flex-1 flex flex-col justify-between space-y-2">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Uploaded {new Date(item.uploadedAt).toLocaleDateString()}</p>
                </div>
